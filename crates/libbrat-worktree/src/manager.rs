...
pub struct WorktreeManager {
    ...
    #[allow(dead_code)]
    repo_root: PathBuf,
    /// Root directory for worktrees (e.g., `.gritee/worktrees`).
...
impl WorktreeManager {
    ...
    /// * `worktree_root` - Relative path for worktrees (e.g., `.gritee/worktrees`).
...
mod tests {
    ...
    fn test_extract_session_id_from_path() {
        assert_eq!(
            extract_session_id_from_path(Path::new("/repo/.gritee/worktrees/s-20250117-a2f9")),
        ...
        assert_eq!(
            extract_session_id_from_path(Path::new("/repo/.gritee/worktrees/not-a-session")),
            None
        );
