//! Continue engine for spawning and controlling Continue.dev CLI sessions.
//!
//! Continue.dev is an open-source AI code assistant with a CLI (`cn`)
...
/// Continue engine for spawning Continue.dev CLI sessions.
///
/// Uses `cn -p` for headless execution with various LLM providers.
...
///
/// - `CONTINUE_MODEL` - Model to use (e.g., "claude-3-opus")
/// - `CONTINUE_FILES` - Glob pattern for files to include
/// - `CONTINUE_CONFIG` - Path to config file
///
/// # Example
///
/// ```no_run
/// use libbrat_engine::{Engine, ContinueEngine, SpawnSpec};
///
/// #[tokio::main]
/// async fn main() -> Result<(), Box<dyn std::error::Error>> {
///     let engine = ContinueEngine::new();
///
///     let spec = SpawnSpec::new("Implement error handling for the API")
///         .env("CONTINUE_MODEL", "claude-3-opus");
///
...
/// }
/// ```
pub struct ContinueEngine {
    /// Active sessions.
    sessions: Arc<RwLock<HashMap<String, SessionState>>>,
}

impl ContinueEngine {
    /// Create a new Continue engine.
    pub fn new() -> Self {
        Self {
    ...
    fn generate_session_id() -> String {
        ...
        format!("continue-{}-{:04x}", ts, rand)
    }

    /// Build the cn command string from the spawn spec.
    fn build_command(&self, spec: &SpawnSpec) -> String {
        let escaped_prompt = spec.command.replace('\'', "'\\''");
        let mut cmd = format!("cn -p '{}' --no-stream", escaped_prompt);

        // Add model if specified in env
        if let Some(model) = spec.env.get("CONTINUE_MODEL") {
            cmd.push_str(&format!(" --model {}", model));
        }

        // Add config file if specified
        if let Some(config) = spec.env.get("CONTINUE_CONFIG") {
            cmd.push_str(&format!(" --config {}", config));
        }

        // Add include files pattern if specified
        if let Some(files) = spec.env.get("CONTINUE_FILES") {
            cmd.push_str(&format!(" --include {}", files));
        }

        // JSON output if requested
        if spec.env.get("CONTINUE_JSON").is_some() {
            cmd.push_str(" --json");
        }
    ...
    fn spawn_output_collector(
        ...
        std::thread::spawn(move || {
            ...
            for line in reader.lines() {
                match line {
                    Ok(text) => {
                        debug!(session_id = %session_id, "continue output: {}", text);
                    ...
                    Err(e) => {
                        warn!(session_id = %session_id, "error reading continue output: {}", e);
                        break;
                    }
                }
            }
            debug!(session_id = %session_id, "continue output stream closed");
        });
    }
    ...
    fn spawn_stderr_collector(session_id: String, child_stderr: std::process::ChildStderr) {
        std::thread::spawn(move || {
            ...
            for line in reader.lines() {
                match line {
                    Ok(text) => {
                        warn!(session_id = %session_id, "continue stderr: {}", text);
                    }
                    Err(e) => {
                        warn!(session_id = %session_id, "error reading continue stderr: {}", e);
                        break;
                    }
                }
            }
...
impl Default for ContinueEngine {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Engine for ContinueEngine {
    async fn spawn(&self, spec: SpawnSpec) -> Result<SpawnResult, EngineError> {
        let session_id = Self::generate_session_id();
        info!(session_id = %session_id, working_dir = ?spec.working_dir, "spawning continue session");

        // Build the cn command
        let shell_cmd = self.build_command(&spec);
        info!(session_id = %session_id, shell_cmd = %shell_cmd, "continue command");
        ...
        let mut child = cmd.spawn().map_err(|e| {
            EngineError::SpawnFailed(format!("failed to spawn cn: {}", e))
        })?;

        let pid = child.id();
        info!(session_id = %session_id, pid = pid, "continue process spawned");

        // Take stdout for output collection
        let stdout = child.stdout.take().ok_or_else(|| {
            EngineError::SpawnFailed("failed to capture continue stdout".to_string())
        })?;
...
mod tests {
    ...
    #[test]
    fn test_continue_engine_creation() {
        let engine = ContinueEngine::new();
    ...
    fn test_build_command_basic() {
        let engine = ContinueEngine::new();
        let spec = SpawnSpec::new("Implement feature");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("cn -p"));
        assert!(cmd.contains("--no-stream"));
        assert!(cmd.contains("'Implement feature'"));
    }

    #[test]
    fn test_build_command_with_model() {
        let engine = ContinueEngine::new();
        let spec = SpawnSpec::new("Task").env("CONTINUE_MODEL", "claude-3-opus");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("--model claude-3-opus"));
    }

    #[test]
    fn test_build_command_with_config() {
        let engine = ContinueEngine::new();
        let spec = SpawnSpec::new("Task").env("CONTINUE_CONFIG", "./config.json");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("--config ./config.json"));
    }

    #[test]
    fn test_build_command_with_files() {
        let engine = ContinueEngine::new();
        let spec = SpawnSpec::new("Review").env("CONTINUE_FILES", "src/**/*.ts");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("--include src/**/*.ts"));
    }

    #[test]
    fn test_build_command_with_json() {
        let engine = ContinueEngine::new();
        let spec = SpawnSpec::new("Task").env("CONTINUE_JSON", "1");
        let cmd = engine.build_command(&spec);
    ...
    fn test_session_id_format() {
        let id = ContinueEngine::generate_session_id();
        assert!(id.starts_with("continue-"));
    }
}
