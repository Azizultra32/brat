//! Copilot engine for spawning and controlling GitHub Copilot CLI sessions.
//!
//! GitHub Copilot CLI provides AI-powered suggestions for shell commands,
//! git operations, and code explanations via the `gh copilot` command.
//!
...
/// Copilot engine for spawning GitHub Copilot CLI sessions.
///
/// Uses `gh copilot suggest` for headless command suggestions.
...
/// - GitHub CLI command suggestions
/// - Code explanation
///
/// # Prerequisites
///
...
///
/// - `COPILOT_TARGET` - Target type: "shell", "git", or "gh" (default: "shell")
///
/// # Limitations
///
/// - Not a full coding agent (no file editing)
/// - Best for shell command generation
/// - Requires GitHub Copilot subscription
///
/// # Example
///
/// ```no_run
/// use libbrat_engine::{Engine, CopilotEngine, SpawnSpec};
///
/// #[tokio::main]
/// async fn main() -> Result<(), Box<dyn std::error::Error>> {
///     let engine = CopilotEngine::new();
///
///     let spec = SpawnSpec::new("find all TypeScript files modified today")
///         .env("COPILOT_TARGET", "shell");
///
...
impl CopilotEngine {
    ...
    fn generate_session_id() -> String {
        ...
        format!("copilot-{}-{:04x}", ts, rand)
    }

    /// Build the gh copilot command string from the spawn spec.
    fn build_command(&self, spec: &SpawnSpec) -> String {
        let escaped_prompt = spec.command.replace('\'', "'\\''");

        // Determine suggestion type (shell, git, or gh)
        let target_type = spec
            .env
            .get("COPILOT_TARGET")
            .map(|s| s.as_str())
            .unwrap_or("shell");
    ...
    fn spawn_output_collector(
        ...
        std::thread::spawn(move || {
            ...
            for line in reader.lines() {
                match line {
                    Ok(text) => {
                        debug!(session_id = %session_id, "copilot output: {}", text);
                    ...
                    Err(e) => {
                        warn!(session_id = %session_id, "error reading copilot output: {}", e);
                        break;
                    }
                }
            }
            debug!(session_id = %session_id, "copilot output stream closed");
        });
    }
    ...
    fn spawn_stderr_collector(session_id: String, child_stderr: std::process::ChildStderr) {
        std::thread::spawn(move || {
            ...
            for line in reader.lines() {
                match line {
                    Ok(text) => {
                        warn!(session_id = %session_id, "copilot stderr: {}", text);
                    }
                    Err(e) => {
                        warn!(session_id = %session_id, "error reading copilot stderr: {}", e);
                        break;
                    }
                }
            }
...
impl Engine for CopilotEngine {
    async fn spawn(&self, spec: SpawnSpec) -> Result<SpawnResult, EngineError> {
        ...
        info!(session_id = %session_id, working_dir = ?spec.working_dir, "spawning copilot session");

        // Build the gh copilot command
        let shell_cmd = self.build_command(&spec);
        info!(session_id = %session_id, shell_cmd = %shell_cmd, "copilot command");
        ...
        let mut child = cmd.spawn().map_err(|e| {
            EngineError::SpawnFailed(format!("failed to spawn gh copilot: {}", e))
        })?;

        let pid = child.id();
        info!(session_id = %session_id, pid = pid, "copilot process spawned");

        // Take stdout for output collection
        let stdout = child.stdout.take().ok_or_else(|| {
            EngineError::SpawnFailed("failed to capture copilot stdout".to_string())
        })?;
...
mod tests {
    ...
    #[test]
    fn test_copilot_engine_creation() {
        let engine = CopilotEngine::new();
        assert!(engine.sessions.read().unwrap().is_empty());
    }

    #[test]
    fn test_build_command_default_target() {
        let engine = CopilotEngine::new();
        let spec = SpawnSpec::new("find large files");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("gh copilot suggest"));
        assert!(cmd.contains("-t shell"));
        assert!(cmd.contains("'find large files'"));
    }

    #[test]
    fn test_build_command_git_target() {
        let engine = CopilotEngine::new();
        let spec = SpawnSpec::new("squash commits").env("COPILOT_TARGET", "git");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("-t git"));
    }

    #[test]
    fn test_build_command_gh_target() {
        let engine = CopilotEngine::new();
        let spec = SpawnSpec::new("create pr").env("COPILOT_TARGET", "gh");
        let cmd = engine.build_command(&spec);
        assert!(cmd.contains("-t gh"));
    }

    #[test]
    fn test_session_id_format() {
        let id = CopilotEngine::generate_session_id();
